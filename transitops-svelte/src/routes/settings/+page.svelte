		...
		<div class="bg-card border border-border rounded-xl p-5">
			...
			<div class="flex items-center justify-between">
				...
				<button
					...
					class="w-11 h-6 rounded-full transition-colors relative {appState.isDark ? 'bg-primary' : 'bg-switch-background'}"
				>
					<span
						class="absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform flex items-center justify-center {appState.isDark ? 'translate-x-5' : 'translate-x-0.5'}"
					>
						{#if appState.isDark}
